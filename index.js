...
function insertarRegistroEnBaseDeDatos() {
  ...
  // Ajustar la hora para la zona horaria de México (GMT-5)
  fechaHoraActual.setHours(fechaHoraActual.getHours() - 6);

  // Formatear la fecha y hora en el formato ISO 8601
  var fechaHoraISO = fechaHoraActual.toISOString();

  // Realizar la inserción en la base de datos usando la API Fetch y Supabase
  fetch("https://ysyfgxarumdyuezphqla.supabase.co/rest/v1/feeding_times", {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({ datetime: fechaHoraISO }),
  })
    .then((response) => {
      if (!response.ok) {
